...
void InstagramStream::serviceGrams(string url)
{
	...
	while( ! mCanceled ) {
        ...
        JsonTree::ConstIter xo = (resultIt);
        

        string url = "";

        if (xo->hasChild("videos.standard_resolution.url")) {
          url   = xo->getChild("videos.standard_resolution.url").getValue();
          cinder::qtime::MovieLoader movieLoader = cinder::qtime::MovieLoader();
   				mBuffer.pushFront( Instagram( userName, url, movieLoader ) );
        }
        else if (xo->hasChild("images.standard_resolution.url")) {
          url   = xo->getChild("images.standard_resolution.url").getValue();
          image = loadImage( loadUrl( url ) );
          // mBuffer.pushFront( Instagram( userName, url, image ) );
        }
